...
public class EditTaskCommandParserTest {
    ...
    public void parse_missingParts_failure() {
        // no index specified
        CommandParserTestUtil.assertParseFailure(parser, VALID_DESC_PAPERWORK, MESSAGE_INVALID_FORMAT);
    ...
    public void parse_invalidPreamble_failure() {
        // negative index
        CommandParserTestUtil.assertParseFailure(parser, "-5" + DESC_PAPERWORK, MESSAGE_INVALID_FORMAT);

        // zero index
        CommandParserTestUtil.assertParseFailure(parser, "0" + DESC_PAPERWORK, MESSAGE_INVALID_FORMAT);
    ...
    public void parse_invalidValue_failure() {
        ...
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_DESC, Description.MESSAGE_CONSTRAINTS); // invalid description
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_DATE, DateTime.MESSAGE_DATE_CONSTRAINTS); // invalid date
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_TIME, DateTime.MESSAGE_TIME_CONSTRAINTS); // invalid time
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_RECUR, Recurrence.MESSAGE_CONSTRAINTS); // invalid recurrence

        // invalid date followed by valid time
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_DATE + INVALID_TIME, DateTime.MESSAGE_DATE_CONSTRAINTS);

        // valid date followed by invalid date. The test case for invalid date followed by valid date
        // is tested at {@code parse_invalidValueFollowedByValidValue_success()}
        CommandParserTestUtil.assertParseFailure(parser, "1" + DATE_DESC_JAN + INVALID_DATE, DateTime.MESSAGE_DATE_CONSTRAINTS);

        // invalid time followed by valid recurrence
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_TIME + RECUR_DAY, DateTime.MESSAGE_TIME_CONSTRAINTS);

        // valid time followed by invalid time. The test case for invalid time followed by valid time
        // is tested at {@code parse_invalidValueFollowedByValidValue_success()}
        CommandParserTestUtil.assertParseFailure(parser, "1" + TIME_DESC_SEVENPM + INVALID_TIME, DateTime.MESSAGE_TIME_CONSTRAINTS);

        // invalid description followed by valid recurrence
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_DESC + RECUR_DAY, Description.MESSAGE_CONSTRAINTS);

        // valid description followed by invalid description. The test case for invalid description
        // followed by valid description
        // is tested at {@code parse_invalidValueFollowedByValidValue_success()}
        CommandParserTestUtil.assertParseFailure(parser, "1" + DESC_PAPERWORK + INVALID_DESC, Description.MESSAGE_CONSTRAINTS);

        // while parsing {@code PREFIX_NAME} alone will reset the tags of the {@code Task} being edited,
        // parsing it together with a valid tag results in error
        CommandParserTestUtil.assertParseFailure(parser, "1" + NAME_DESC_AMY + NAME_DESC_ALEX + NAME_EMPTY,
                Name.MESSAGE_CONSTRAINTS);
        CommandParserTestUtil.assertParseFailure(parser, "1" + NAME_DESC_AMY + NAME_EMPTY + NAME_DESC_ALEX,
                Name.MESSAGE_CONSTRAINTS);
        CommandParserTestUtil.assertParseFailure(parser, "1" + NAME_EMPTY + NAME_DESC_AMY + NAME_DESC_ALEX,
                Name.MESSAGE_CONSTRAINTS);

        // multiple invalid values, but only the first invalid value is captured
        CommandParserTestUtil.assertParseFailure(parser, "1" + INVALID_DATE + INVALID_RECUR + DESC_PAPERWORK
