...
* Entering the same elderly name multiple times in one command will not throw an error but the elderly will only be added to the task once.
...
* Undo-ing a `deleteElderly` or `editElderly` command would restore the elderly's name in relevant tasks but the order of names may be different.
...
* Redo-ing the `editElderly` command would restore the elderly's name in relevant tasks but the order of names may be different.
