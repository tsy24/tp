...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the elderly identified "
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of elderly data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the elderly’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a elderly.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.elderly`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each elderly.

Simply add the following to [`seedu.address.ui.ElderlyCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
``` java
public PersonCard(Person elderly, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person elderlyToEdit = lastShownList.get(index.getZeroBased());
        Person editedElderly = new Person(
                elderlyToEdit.getName(), elderlyToEdit.getPhone(), elderlyToEdit.getEmail(),
                elderlyToEdit.getAddress(), remark, elderlyToEdit.getTags());

        model.setPerson(elderlyToEdit, editedElderly);
     ...
     * {@code elderlyToEdit}.
     */
    private String generateSuccessMessage(Person elderlyToEdit) {
